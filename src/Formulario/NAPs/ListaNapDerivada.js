...
const ListaNapDerivada = () => {
    ...
    const [Distribucion2Nivel, setDistribucion2Nivel] = useState("");

    const CargarDatos=()=>{
        axios.get('https://localhost:7097/api/ControladorDatos/NapsDerivadas')
        .then(response => {
    ...
    const BorrarSeleccionado = () => {
        ...
        axios.delete(`https://localhost:7097/api/ControladorDatos/BorrarNapDerivada/${NAPBorrar}`)
        .then(response=>{
    ...
    const handleGuardar=()=>{
        ...
        if (!codigoNap || !ubicacion || !distanciaOptica || !Distribucion2Nivel){
            Swal.fire({
                icon: 'error',
                title: 'Error',
            ...
            axios.post("https://localhost:7097/api/ControladorDatos/CrearNapDerivada",nap,{
                headers:{
    ...
    return (
        <div>
                    ...
                    <table id="example" className="table" style={{ width: '100%' }}>
                        ...
                        <tbody>
                            {paginatedTr.length > 0 ? (
                                paginatedTr.map(nap => (
                                    <tr key={nap.idNAPDerivada}>
                                        <td>{nap.idNAPDerivada}</td>
                                        <td>{nap.codigoNap}</td>
                                        <td>{nap.ubicacion}</td>
                                        <td>{nap.distanciaOptica}</td>
                                        <td style={{ padding: '10px' }}>
                                            <i onClick={()=>BorrarClick(nap.idNAPDerivada)} className="bi bi-trash" style={{ padding: '5px', color: '#E58A92' }}></i>
            ...
            <Modal show={ShowDetalle} onHide={() => setShowDetalle(false)}>
                ...
                <Modal.Body>
                    <p><strong>Id:</strong> {detalleNAP.idNAPDerivada}</p>
                    ...
                    <p><strong>Distribucion:</strong> {detalleNAP.distribucion2Nivel}</p>
                </Modal.Body>
                <Modal.Footer>
                ...
                <Modal.Body>
                    <Form>
                        ...
                        <Form.Group className='mb-3'>
                                ...
                                <Select
                                    value={distribuciones.find(option => option.value === Distribucion2Nivel)} // Esto mantiene el valor seleccionado
                                    onChange={(selectedOption) => setDistribucion2Nivel(selectedOption.value)} // Obtén solo el valor
                                    options={distribuciones.map((item) => ({
                                        value: item.idDistribucion2Nivel,
...
export default ListaNapDerivada;
